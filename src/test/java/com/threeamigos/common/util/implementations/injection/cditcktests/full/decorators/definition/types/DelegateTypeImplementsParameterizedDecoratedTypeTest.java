...
package com.threeamigos.common.util.implementations.injection.cditcktests.full.decorators.definition.types;
...
class DelegateTypeImplementsParameterizedDecoratedTypeTest {
    ...
    void testDelegateTypeIsValid() {
        ...
        syringe.setup();
        try {
            MockLogger logger = getContextualReference(syringe.getBeanManager(), MockLogger.class);
            logger.log("foo");
            assertEquals("Decorated:foo", MockLogger.getLog());
        } finally {
            syringe.shutdown();
        }
    }
