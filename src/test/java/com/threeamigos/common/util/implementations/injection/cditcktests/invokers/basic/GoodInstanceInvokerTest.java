...
class GoodInstanceInvokerTest {
    ...
    void test() throws Exception {
        ...
        try {
            ...
            assertEquals("foobar_1", hello.invoke(syringe.inject(MyService.class), new Object[] { "1" }));
            assertEquals("foobar_2", hello.invoke(new MyService(), new Object[] { "2" }));

            Invoker<MyServiceInterface, String> helloInterface = invokers.get("helloInterface");
            assertEquals("quux_3", helloInterface.invoke(syringe.inject(MyServiceInterface.class), new Object[] { "3" }));
            assertEquals("quux_4", helloInterface.invoke(new MyService(), new Object[] { "4" }));
        } finally {
            syringe.shutdown();
        }
    }
