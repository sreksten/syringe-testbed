 ...
 */
package com.threeamigos.common.util.implementations.injection.cditcktests.full.decorators.invocation.observer;
...
class DecoratorInvocationTest {
    ...
    void testDecoratorInvocation() {
        ...
        addDecoratorBeansXml(syringe, ObserverDecorator.class);

        syringe.setup();
        try {
            ObserverImpl.reset();
            syringe.getBeanManager().getEvent().select(Foo.class).fire(new Foo("foo!"));
            assertTrue(ObserverImpl.isObservervedCorrectly());
        } finally {
            syringe.shutdown();
        }
    }
