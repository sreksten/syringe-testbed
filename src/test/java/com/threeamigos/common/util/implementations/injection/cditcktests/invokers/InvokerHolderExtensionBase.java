 ...
 */
package com.threeamigos.common.util.implementations.injection.cditcktests.invokers;
...
public abstract class InvokerHolderExtensionBase {
    private final Map<String, InvokerInfo> invokers = new LinkedHashMap<String, InvokerInfo>();

    protected final void registerInvokers(BeanInfo bean, InvokerFactory invokers, Set<String> methods) {
        registerInvokers(bean, invokers, methods, new Consumer<InvokerBuilder<?>>() {
            @Override
            public void accept(InvokerBuilder<?> builder) {
            }
        });
    }

    protected final void registerInvokers(BeanInfo bean,
                                          InvokerFactory invokers,
                                          Set<String> methods,
    ...
    protected final void synthesizeInvokerHolder(SyntheticComponents syn) {
        syn.addBean(InvokerHolder.class)
            ...
            .withParam("names", invokers.keySet().toArray(new String[invokers.size()]))
            .withParam("invokers", invokers.values().toArray(new InvokerInfo[invokers.size()]))
