...
public final class AnnotatedTypes {
    ...
    public static boolean compareAnnotatedTypes(AnnotatedType<?> t1, AnnotatedType<?> t2) {
        ...
        for (AnnotatedField<?> field : t2.getFields()) {
            fields.put(field.getJavaMember(), field);
        }
        for (AnnotatedField<?> field : t1.getFields()) {
            if (fields.containsKey(field.getJavaMember())) {
                if (!compareAnnotatedField(field, fields.get(field.getJavaMember()))) {
                    return false;
                }
            } else {
                return false;
        ...
        for (AnnotatedMethod<?> method : t2.getMethods()) {
            methods.put(method.getJavaMember(), method);
        }
        for (AnnotatedMethod<?> method : t1.getMethods()) {
            if (methods.containsKey(method.getJavaMember())) {
                if (!compareAnnotatedCallable(method, methods.get(method.getJavaMember()))) {
                    return false;
                }
            } else {
                return false;
        ...
        for (AnnotatedConstructor<?> constructor : t2.getConstructors()) {
            constructors.put(constructor.getJavaMember(), constructor);
        }
        for (AnnotatedConstructor<?> constructor : t1.getConstructors()) {
            if (constructors.containsKey(constructor.getJavaMember())) {
                if (!compareAnnotatedCallable(constructor, constructors.get(constructor.getJavaMember()))) {
                    return false;
                }
            } else {
                return false;
