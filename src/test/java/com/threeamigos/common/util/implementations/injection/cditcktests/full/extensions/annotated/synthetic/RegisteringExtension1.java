...
public class RegisteringExtension1 implements Extension {
    ...
    void registerApple(@Observes BeforeBeanDiscovery event, final BeanManager manager) {
        ...
        AnnotatedType<Apple> apple = new AnnotatedTypeWrapper<Apple>(
                manager.createAnnotatedType(Apple.class),
                false,
                Juicy.Literal.INSTANCE
        );
        event.addAnnotatedType(apple, RegisteringExtension1.class.getName() + "." + Apple.class.getName());

        AnnotatedType<Orange> orange = new AnnotatedTypeWrapper<Orange>(
                manager.createAnnotatedType(Orange.class),
                false,
                Juicy.Literal.INSTANCE
        );
        event.addAnnotatedType(orange, RegisteringExtension1.class.getName() + "." + Orange.class.getName());
